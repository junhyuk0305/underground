 ...
 * ========================================================= */
// [archetype 이름, 업종, 시설[], 유휴요일, 시작, 종료, slot, program후보[], mx, my]
const CANON_VENUES = [
...
const CANON_DIM = [
  ['유휴 서재','북카페',['좌석 8','빔프로젝터','콘센트 넉넉'],'평일','15:00','18:00',120,['글쓰기 모임','스터디','보드게임'],40,40],
  ['비어있는 작업카페','카페',['2인 테이블 6','통창','콘센트 넉넉','디저트'],'평일','14:00','17:00',120,['모각작','드로잉 클래스','커피 클래스'],82,66],
  ['동네 공유주방','음식점',['조리대 2','식기 완비','12인석'],'평일','15:00','18:00',120,['쿠킹 클래스','저녁 소셜','원데이 베이킹'],14,80],
];
...
function regionStimulus(region) {
  ...
  CANON_VENUES.forEach((cv, i) => {
    const [name, category, facilities, idle_days, idle_start, idle_end, slot_minutes, program_candidates, mx, my] = cv;
    ...
    venues.push({ id: vid, region_id: region.id, name, address: `${region.name} 일대`, category,
      ...
      mx, my, lat, lng, images: [], facilities, program_candidates });
  ...
  CANON_DIM.forEach((cv, i) => {
    ...
    const lng = region.lng != null ? region.lng + (mx - 50) / 2000 : null;
    venues.push({ id: vid, region_id: region.id, name, address: `${region.name} 일대`, category,
      capacity: 8 + i * 2, idle_days, idle_start, idle_end, slot_minutes, mx, my, lat, lng, images: [], facilities, program_candidates });
  });
