...
async function beginOwnerDemo(){
  ...
  try{
    ...
    go('#/owner-home');
  }catch(e){ toast(e.message,'err'); go('#/signup'); }
}
...
function renderSignupStep(){
  ...
  if(su.step===3){ // 지역·거주
    fscr(`${stepDots(3,4)}<h1 class="dtitle">어느 동네에 사세요?</h1>
      <p class="hint" style="margin-bottom:16px">서울은 구, 그 외 지역은 시 단위로 열려요.</p>
...
async function screenMap(){
  ...
  renderAppbar({title:'언더그라운드맵', brand:true, city:regionName(rid)}); renderTabbar('map'); setAdsVisible(true);
  ...
  $screen.innerHTML = `
    <div class="mapfull">
      ...
      <div class="map-float">
        ...
        ${visitor}
        <div class="map-legend"><span><i style="background:var(--brand);box-shadow:0 0 8px rgba(255,78,0,.85)"></i>진행중 ${liveN}</span><span><i style="background:#5a5a5a"></i>빈 공간</span></div>
      </div>
...
function venueCard(v){
  ...
  return `<div class="mcard mcard--tap" data-v="${v.id}">
    <div class="top"><div class="mt">${esc(v.name)}</div><span class="badge badge--closed">공간만</span></div>
    <div class="venue">${ICON.store}${esc(v.category||'')} · 유휴 ${esc(v.idle_start||'')}~${esc(v.idle_end||'')}</div>
    <div class="hint">아직 열린 프로그램이 없어요 · 호스트가 되어 열 수 있어요</div></div>`;
}
...
async function screenHostApply(venueId){
  ...
  scrRaw(`<div class="screen__pad" style="padding-bottom:100px">
    ...
    <div class="field"><label>매장 분배율(%) <span class="sub">· 최소 30</span></label><input class="input" id="share" type="number" min="30" max="100" value="40"></div>
    <div class="hl"><span>${ICON.won}</span><div class="t">참가비의 일부는 공간을 내어준 매장에 <b>꼭 돌아가요(최소 30%)</b>.</div></div>
    </div>
  ...
  document.getElementById('submit').onclick=async()=>{
    ...
    if(share<30) return toast('매장 분배율은 최소 30% 이상이어야 해요.','err');
...
async function screenMy(){
  ...
  if(state.mySeg==='join'){
    ...
    body.innerHTML = hs.length? hs.map(m=>`<div class="mcard mcard--tap" data-v="${m.venue_id}"><div class="top"><div class="mt" style="font-size:15px">${esc(m.title)}</div>${hb(m)}</div><div class="venue">${ICON.store}${esc(m.venue_name||'')} · ${fmtShort(m.starts_at)}</div></div>`).join('') : `<div class="empty"><div class="ic">✚</div>아직 연 모임이 없어요.<br>지도의 빈 공간에서 프로그램을 열어보세요.</div>`;
  }
...
async function screenMe(){
  ...
  // 사장님 진입/전환 카드
  const ownerRow = state.ownerMode
    ? `<div class="lrow lrow--tap" id="toParticipant">${iconChip(ICON.map)}<div><div class="tt">참여자 모드로 전환</div><div class="ss">지도·모임 둘러보기</div></div><span class="chev">›</span></div>`
    : `<div class="lrow lrow--tap" id="toOwner">${iconChip(ICON.store)}<div><div class="tt">${isOwner?'사장님 모드':'사장님이신가요?'}</div><div class="ss">${isOwner?'내 매장·요청·정산 관리':'유휴 시간에 공간을 열고 수익 만들기'}</div></div><span class="chev">›</span></div>`;
...
async function screenOwnerHome(){
  ...
  if(!d.venues.length){
    body.innerHTML=`<div class="owner-hero"><div class="owner-hero__k">${ICON.store}</div>
      ...
      <div class="kpis kpis--3">
        <div class="kpi"><div class="kpi__v">5%</div><div class="kpi__l">플랫폼 수수료<br>(호스트 참가비 기준)</div></div>
  ...
  body.innerHTML=`
    <div class="owner-banner"><span class="ob-badge">사장님 모드</span><div class="ob-name">${esc(v.name)}</div><div class="ob-sub">${esc(v.category||'')} · ${idle}</div></div>
    <div class="kpis kpis--3">
    ...
    <div class="hl"><span>${ICON.won}</span><div class="t">참가비의 <b>최소 ${v.min_share_pct||30}%</b>가 매장에 돌아가도록 설정돼 있어요. 요청마다 분배율을 보고 수락 여부를 정하세요.</div></div>
    ${ownerCtaBlock()}`;
...
async function screenVenueEdit(venueId){
  ...
  let v={ name:'', category:'', address:'', capacity:8, idle_days:'평일', idle_start:'15:00', idle_end:'17:00', slot_minutes:120, min_share_pct:30, facilities:[], program_candidates:[], house_rules:'' };
  ...
  fscr(`<h1 class="dtitle" style="margin-top:4px">${editing?'매장 정보를 다듬어요':'우리 매장, 이렇게 열어요'}</h1>
    ...
    <div class="row2"><div class="field"><label>1회 이용(분)</label><input class="input" id="vslot" type="number" min="30" step="30" value="${v.slot_minutes||120}"></div>
      <div class="field"><label>최소 매장 분배율(%)</label><input class="input" id="vshare" type="number" min="0" max="100" value="${v.min_share_pct||30}"></div></div>
...
async function screenOwnerSettlement(){
  ...
  body.innerHTML=`
    ...
    <div class="hl"><span>${ICON.won}</span><div class="t">이 모임이 만석이면 매장에 이만큼 돌아가요. 참가비를 쪼개는 게 아니라, <b>손님이 늘고 그 손님이 매장에서 씁니다.</b></div></div>
...
function ownerCtaBlock(){
  return `<div class="owner-cta">
    <button class="btn btn--md btn--soft btn--block" id="oIntent">👀 이 정도면 관심 간다</button>
    <button class="btn btn--lg btn--primary btn--block" id="oClose" style="margin-top:10px">이번 주 무료 시범 1타임 열어보기</button>
  </div>`;
}
...
function surveyQuestions(role){
  if(role==='owner') return [
    { id:'o1', type:'scale5', q:'이 서비스, 우리 가게 유휴시간에 실제로 도움이 될까요?', lo:'전혀', hi:'매우' },
    { id:'o2', type:'single',  q:'가장 끌린 점은?', opts:['손님 유입','데드타임 매출','가게 홍보','단골 확보','딱히 없음'] },
...
function screenSurvey(){
  ...
  const role = STUDY_ROLE;
  const qs = surveyQuestions(role);
  const ans = {};
  const intro = role==='owner'
    ? '딱 5개만요. 사장님 답이 다음 사장님을 설득할 근거가 돼요.'
  ...
  const prog=()=>{ const n=qs.filter(q=>ans[q.id]!=null && ans[q.id]!=='').length; const el=document.querySelector('#svProg span'); if(el) el.style.width=Math.round(n/qs.length*100)+'%'; };
  qs.forEach(q=>bindQuestion(list, q, ans, prog));
  const submit=(skipped)=>{
    const nps  = role==='owner' ? null : (ans['q5']!=null ? +ans['q5'] : null);
    const open = role==='owner' ? (ans['o5']||'') : (ans['q10']||'');
...
function screenResults(){
  ...
  const ag=document.getElementById('again'); if(ag) ag.onclick=async()=>{ resetStudy(); clearFlags(); try{ await db.signOut(); }catch(e){} state.session=null; state.profile=null; state.viewRegion=null; state.viewable=[]; go('#/study'); };
}
...
function computeMetrics(all){
  ...
  supply.o1avg  = supply.o1n ? (supply.o1sum/supply.o1n) : null;
...
const AUTHED=['map','venue','host-apply','joined','my','passes','me',...OWNER_ROUTES];
